...
#  quit()
  ...
  if do_atp:
    ...
    with h5py.File(h5file, 'r') as f:
      for grp_name, grp in f.items():
        ...
        atp_fc = np.zeros(dlen)
        atp_ud = np.zeros(dlen)
        atp_G = np.zeros(dlen)
        atp_T = np.zeros(dlen)
        ...
        grp['Fc'].read_direct (atp_fc)
        grp['Ud'].read_direct (atp_ud)
        grp['G'].read_direct (atp_G)
        grp['T'].read_direct (atp_T)
  ...
  if do_atp:
    ax[0,0].plot (atp_t, atp_G, label='ATP G')
    ax[0,0].plot (atp_t, 10.0*atp_T, label='ATP T')
  ...
  if do_atp:
    ax[0,1].plot (atp_t, atp_fc/60.0, label='ATP Fc')
    ax[0,1].plot (atp_t, atp_ud, label='ATP Ud')
